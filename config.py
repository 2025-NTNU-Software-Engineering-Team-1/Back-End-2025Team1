...
CORS_ALLOWED_ORIGINS = [
    o.strip()
    for o in os.getenv('CORS_ALLOWED_ORIGINS',
                       'http://localhost:*,http://127.0.0.1:*').split(',')
    if o.strip()
]

# Force Secure flag on cookies (for HTTPS production)
FORCE_SECURE_COOKIES = os.getenv('FORCE_SECURE_COOKIES',
                                 'false').lower() == 'true'
...
RATE_LIMIT_LOCKOUT_SECONDS = int(os.getenv('RATE_LIMIT_LOCKOUT_SECONDS',
                                           '900'))
