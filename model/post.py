...
@Request.json('course: str?', 'title: str?', 'content: str?', 'target_thread_id: str?',
              'contains_code: bool?', 'problem_id: int?')
@login_required
def modify_post(user, course, title, content, target_thread_id, contains_code, problem_id):
    if course == 'Public':
        return HTTPError('You can not add post in system.', 403)

    err_msg, code = Post.modify_post_logic(
        request.method, user, course, target_thread_id, 
