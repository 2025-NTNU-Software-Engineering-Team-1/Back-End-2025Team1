...
class Post:
    ...
    @classmethod
    def modify_post_logic(cls, method, user, course_name, target_thread_id,
                          title, content, contains_code, problem_id):
        target_course = None
        target_thread = None
        ...
        if course_name:
            ...
            if target_thread.status:
                return "Forbidden,the post/reply is deleted.", 403
        ...
        if method == 'POST':
            # 截止日期檢查
            err_msg = cls._check_deadline_guard(target_course, user_ref,
            ...
            err = cls.edit_post(target_thread, user, content, title,
                                capability)
    ...
    def update_status_logic(cls, user, post_id, action):
        ...
        if user.role not in (engine.User.Role.ADMIN, engine.User.Role.TEACHER,
                             engine.User.Role.TA):
    ...
    @classmethod
    def _check_deadline_guard(cls, target_course, user_ref, contains_code,
                              problem_id):
        if not (contains_code and problem_id): return None
        capability = target_course.own_permission(user_ref)
        if bool(capability & (CourseWrapper.Permission.GRADE
                              | CourseWrapper.Permission.MODIFY)):
            return None
    ...
    def get_problem_deadline(cls, problem_id):
        ...
        p_obj = engine.Problem.objects(
            problem_id=pid).first() or engine.Problem.objects(pk=pid).first()
        if p_obj:
            return getattr(p_obj, 'deadline', None) or getattr(
                p_obj, 'Deadline', None)
        return None
    ...
    def found_thread(cls, target_thread):
        return {
            'Id':
            str(target_thread.id),
            'Content':
            target_thread.markdown,
            'Author':
            target_thread.author.info,
            'Status':
            target_thread.status,
            'Pinned':
            getattr(target_thread, 'pinned', False),
            'Closed':
            getattr(target_thread, 'closed', False),
            'Solved':
            getattr(target_thread, 'solved', False),
            'Created':
            target_thread.created.timestamp(),
            'Updated':
            target_thread.updated.timestamp(),
            'Reply':
            [cls.found_thread(r)
             for r in target_thread.reply] if target_thread.reply else []
        }
    ...
    def found_post(cls, course_obj, target_id=None):
        ...
        for x in course_obj.posts:
            ...
            data.append({
                'thread': cls.found_thread(x.thread),
                'title': x.post_name
            })
        return data
    ...
    def add_reply(cls, target_thread, user, content):
        ...
        now = datetime.now()
        new_thread = engine.PostThread(markdown=content,
                                       course_id=target_thread.course_id,
                                       depth=target_thread.depth + 1,
                                       created=now,
                                       updated=now,
                                       author=user.obj).save()
    ...
    def edit_post(cls,
        ...
        if delete:
            if user.obj != author and not (capability
                                           & CourseWrapper.Permission.GRADE):
                return 'Forbidden'
            target_thread.update(set__status=1,
                                 set__markdown='*Content was deleted.*')
            cls._log_action(user, 'DELETE_POST', target_thread.id)
        else:
            if user.obj != author and not (capability
                                           & CourseWrapper.Permission.MODIFY):
                return 'Forbidden'
            target_thread.update(set__markdown=content,
                                 set__updated=datetime.now())
            cls._log_action(user, 'EDIT_POST', target_thread.id)

        target_post = engine.Post.objects(thread=target_thread).first()
        if target_post:
            target_post.update(
                set__post_name='*The Post was deleted*' if delete else title)

    @classmethod
    def delete_post(cls, target_thread, user, capability):
        return cls.edit_post(target_thread,
                             user,
                             None,
                             None,
                             capability,
